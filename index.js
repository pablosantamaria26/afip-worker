...
async function getReceptorDesdePadron(cuitCliente) {
  try {
    const padron = await afip.RegisterScopeThirteen.getTaxpayerDetails(Number(cuitCliente));
    if (!padron) return { nombre: `CUIT ${cuitCliente}`, domicilioAfip: "", condicionIVA: "IVA Responsable Inscripto" };

    const dg = padron?.datosGenerales || padron;
    const df = padron?.datosGenerales?.domicilioFiscal || padron?.domicilioFiscal;
    ...
    const direccionCompleta = df
      ? [df.direccion, df.localidad, df.codPostal ? `CP: ${df.codPostal}` : ""].filter(Boolean).join(" - ")
      : "";

    return { nombre: nombre || `CUIT ${cuitCliente}`, domicilioAfip: direccionCompleta, condicionIVA: "IVA Responsable Inscripto" };
  } catch (e) {
    return { nombre: `CUIT ${cuitCliente}`, domicilioAfip: "", condicionIVA: "IVA Responsable Inscripto" };
  }
}
...
// ============================
// ✅ LOGO (local) -> DataURL para PDF + CID para Email
// ============================
...
const LOGO_PATH = findLogoPath();          // para email (cid)
const LOGO_DATA_URL = logoPathToDataUrl(LOGO_PATH); // para PDF (siempre anda)
...
// - Soporta req.body.previewParte = 1..N o "ALL"
//   * 1..N => devuelve SOLO esa parte
//   * "ALL" => devuelve un HTML contenedor con iframes (si querés usarlo)
//     (tu index.html también tiene fallback para "ALL" haciendo N llamadas)
// ============================
app.post("/debug/preview", async (req, res) => {
  try {
    const pv = await getPtoVentaSeguro();

    // ✅ NUEVO: parte solicitada (1..N) o "ALL"
    ...
    // 2) factor global (para que total global post-desc sea EXACTO)
    ...
    // ✅ Si te piden ALL, podés devolver un HTML contenedor.
    // (Tu index.html también tiene fallback por si no lo usás)
    if (String(previewParteReq).toUpperCase() === "ALL") {
      ...
      // Armamos cada srcdoc llamando recursivamente al mismo builder (sin re-llamar endpoint)
      const htmlsPorParte = [];
      for (let p = 1; p <= chunks.length; p++) {
    ...
    // 4) preview parte N (o 1 si no llega)
    const idxParte = Math.max(0, Number(previewParteReq || 1) - 1);
    const itemsParte = chunks[idxParte] || [];
    const totalParte = round2(itemsParte.reduce((a, it) => a + Number(it.subtotalConIva || 0), 0));

    // 5) descuento de la parte (para que cuadre con IMPORTE TOTAL)
    let subBrutoParte = 0, descParte = 0, pctParte = 0;
    const factorUsado = (factor > 0 && factor < 1) ? factor : 1;
...
// ============================
// ✅ FACTURAR (PDF + MAIL coherentes por PARTE) + 1 SOLO EMAIL con N PDFs
// ============================
app.post("/facturar", async (req, res) => {
  try {
    ...
    // globales (del /leer-remito o manual)
    const subtotalBrutoIn = Number(req.body.subtotalBruto || 0);
    const descuentoPctIn = Number(req.body.descuentoPct || 0);
    const descuentoImporteIn = Number(req.body.descuentoImporte || 0);
    const totalFinalIn = Number(req.body.total || req.body.totalFinal || 0);
    ...
    // 1) normalizamos items (preservando subtotalConIva si viene)
    let allItems = itemsIn.map((x) => {
      const cantidad = Number(x.cantidad || 0);
      const descripcion = String(x.descripcion || "").trim();
      const precioConIva = Number(x.precioConIva || 0);
    ...
    // 3) factor global para que total post-desc coincida EXACTO
    ...
    // para descuento por parte coherente
    const factorUsado = (factor > 0 && factor < 1) ? factor : 1;
    let acumuladoSubBruto = 0;

    // ============================
    // ✅ EMAIL: 1 SOLO MAIL con N PDFs + logo inline (CID)
    // ============================
    const emailEnabled = !!(GMAIL_USER && GMAIL_APP_PASS);
    const mailParts = [];
    const mailAttachments = [];

    const LOGO_CID = "logo_mlm_cid";
    if (emailEnabled && LOGO_PATH) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      // nota con resumen de partes (igual que preview)
      let notaFactura = "";
      if (chunks.length > 1) {
      ...
      // ✅ descuento de ESTA parte (coherente)
      let subBrutoParte = 0, descParte = 0, pctParte = 0;
      ...
      // ✅ acumulamos parte + pdf adjunto (mail final)
      mailParts.push({
        parte: i + 1,
      ...
      if (emailEnabled) {
        const pdfBuffer = await downloadToBuffer(pdfRes.file);
        mailAttachments.push({
      ...
      resultados.push({ nroFactura: nro, cae: result.CAE, total: impTotal });
    }

    // ============================
    // ✅ Envío 1 SOLO EMAIL con todos los PDFs adjuntos
    // ============================
      ...
      const mailHtml = `
        <div style="font-family:Arial,sans-serif;background:#f6f7fb;padding:30px;">
          <div style="max-width:720px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 8px 22px rgba(0,0,0,0.08);">
            <div style="background:#0f172a;color:#fff;padding:22px 24px;display:flex;align-items:center;gap:14px;">
              ${LOGO_PATH ? `<img src="cid:${LOGO_CID}" alt="Logo" style="height:42px;width:auto;display:block;border-radius:6px;background:#fff;padding:6px;">` : ``}
              <div>
                <div style="font-size:18px;font-weight:900;letter-spacing:0.4px;">${safeText(EMISOR.nombreVisible)}</div>
                <div style="opacity:0.85;font-weight:700;font-size:12px;margin-top:2px;">Factura Electrónica A (AFIP / ARCA)</div>
              </div>
            </div>
...
// ============================
// ✅ Hardening: logs de errores no capturados (te salva cuando algo explota en async)
// ============================
